...
export default function VodCard({ vod }: VodCardProps) {
  ...
  return (
    <Card shadow={'lg'} radius={10} p={0}>
      <Card.Section
        sx={{ backgroundColor: colorScheme === 'dark' ? '#141517' : '#f8f9fa' }}
      >
        <AspectRatio ratio={16 / 9}>
          ...
          {overlayVisible && (
            <Text sx={{ zIndex: 2 }} px={25} size={18} color={'white'}>
              {vod.title}
            </Text>
          )}
          {overlayVisible && (
            <Overlay opacity={0.6} blur={2} color="#000" zIndex={0}>
              hello
            </Overlay>
          )}
        </AspectRatio>
      ...
      <Card.Section px={5}>
        ...
        <Group noWrap position="apart">
          ...
          <Anchor href={`https://twitch.tv/${vod.user_name}`}>
            {vod.user_name}
          </Anchor>
